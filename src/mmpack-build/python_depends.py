...
def _resolve_class_attr(classnode: Union[Instance, ClassDef],
                        attrname: str) -> Tuple[str, NodeNG]:
    """
    lookup in the current class and its anscestors to find which one define the
    attribute

    Args:
        classnode: class definition or instance where to lookup for attribute
        attrname: name of attribute to look

    Returns:
        the couple of public symbol name of the attribute and found class node
    """
    base_node = classnode.local_attr(attrname)[0].parent
    sym = _get_used_attr_name(base_node, attrname)

    return (sym, base_node)


def _resolve_super_attr(super_node: Super,
                        attrname: str) -> Tuple[str, NodeNG]:
    """
    ...
    for classnode in super_node.super_mro():
        classes_with_attr = classnode.local_attr(attrname)
        if not classes_with_attr:
            continue

        base_node = classes_with_attr[0].parent
        sym = _get_used_attr_name(base_node, attrname)
...
def _follow_attr(attr: Attribute, used_symbols: Set[str],
    ...
    # Infer the final type of the attribute
    try:
    ...
    if isinstance(base_node, (Instance, ClassDef)):
        sym, base_node = _resolve_class_attr(base_node, attr.attrname)
    elif isinstance(base_node, Super):
        sym, base_node = _resolve_super_attr(base_node, attr.attrname)
    elif sym:
        sym += '.' + attr.attrname
    else:
        raise AttributeInferenceError
