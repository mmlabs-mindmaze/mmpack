...
def _get_used_attr_name(classdef: ClassDef, attrname: str) -> str:
    pkgname = classdef.qname().split('.', maxsplit=1)[0]
...
def _inspect_attr(attr: Attribute, used_symbols: Set[str], pkgfiles: Set[str]):
    for node in _infer_node_def(attr.expr):
        if isinstance(node, ClassDef):
            classdef = node
            if _is_external_pkg(classdef.root(), pkgfiles):
                used_symbols.add(_get_used_attr_name(classdef, attr.attrname))
