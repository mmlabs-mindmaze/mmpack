...
def _get_classbase_defining_attr(cldef: Union[Instance, ClassDef, Super],
    ...
    # Get the first node that defines an attribute (even in ancestors).
    nodes = cldef.getattr(attrname)
    if not nodes:
        return Uninferable

    # Get the base of the first node that has an inspectable parent (skip
    # NoneType, builtins...)
    for node in nodes:
        try:
            base = nodes[0].frame()
