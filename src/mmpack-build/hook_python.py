...
class PyPkgInfo:
    ...
    def add_file(self, filename: str):
        """
        Add filename to the set of file of python package
        """
...
def _add_qualified_symbol_names(provide: Provide, symbols: Dict[str, str]):
    for pubsym, qname in symbols.items():
        ...
        ver_qname = provide.symbols.get(qname)
        if not ver_qname or Version(ver_pubsym) > Version(ver_qname):
            provide.symbols[qname] = ver_pubsym
