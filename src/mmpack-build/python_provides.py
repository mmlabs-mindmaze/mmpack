...
def _gen_pypkg_symbols(pkgdata: PkgData):
    files = pkgdata.pkgfiles.copy()
    initfiles = [f for f in pkgdata.pkgfiles if basename(f) == '__init__.py']
    for initfile in initfiles:
        # Process first init file
        initmod = MANAGER.ast_from_file(initfile)
        pkgdata.add_module_public_symbols(initmod)
        files.discard(initfile)

        # Process all files that in the package
        subdir = dirname(initfile)
        pkgfiles = {f for f in files if dirname(f) == subdir}
        files.difference_update(pkgfiles)
        for pyfile in pkgfiles:
            pkgdata.add_module_public_symbols(MANAGER.ast_from_file(pyfile))

            # For a python package, __main__.py holds the contents which will
            # be executed when the module is run with -m. Hence we test the
            # capability of a package to be runnable by trying to import the
            # <pypkg>.__main__ module. If the import is possible
            # <pypkg>.__main__ will be added to the public symbols.
            if basename(pyfile) == '__main__.py':
                pkgdata.add_namespace_symbol(initmod.qname(), '__main__')

    # Process all single module file (those at the level of sitedir)
    for pyfile in {f for f in files if dirname(f) in sys.path}:
