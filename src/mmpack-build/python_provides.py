...
class PkgData:
    ...
    def _add_importfrom(self, impfrom: ImportFrom):
        ...
        for imported_name, _ in impfrom.names:
            # If the imported name is not an attribute of the name, the name
            # refer to a module that must be imported
            if not mod.getattr(imported_name):
                imported_mod = impfrom.do_import_module(imported_name)
