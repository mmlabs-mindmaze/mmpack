...
class DebPkg(SysPkg):
    ...
    def parse_from_index(self, fileobj: TextIOBase):
        """
        ...
        for line in fileobj:
            try:
                ...
                attrname = self.desc_field_to_attr.get(key)
                if attrname:
                    self.__setattr__(attrname, data.strip())
            except ValueError:
...
class Dpkg(SysPkgManager):
    ...
    def _parse_pkgindex(self, builddir: str, srcname: str) -> List[SysPkg]:
        ...
        with gzip.open(gzipped_index, 'rt') as index_file:
            pkg = DebPkg()
            while pkg.parse_from_index(index_file):
