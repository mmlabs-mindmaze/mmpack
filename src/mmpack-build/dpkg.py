...
def _download_packages_index(repo_url: str, builddir: str) -> str:
    ...
    packages_url = '{}/dists/stable/main/binary-{}/Packages.gz'\
...
class DebPkg:
    ...
    def parse_from_file(self, fileobj: TextIOBase):
        for line in fileobj:
            try:
    ...
    def fetch_and_extract(self, repo_url: str, localdir: str) -> List[str]:
        # download package
        pkg_url = '{}/{}'.format(base_repo_url, pkg.filename)
        pkg_file = os.path.join(localdir, os.path.basename(pkg.filename))
...
def _parse_packages(pkg_index_path: str, srcpkg: str) -> List[Dict[str, str]]:
    ...
    with open(pkg_index_path, 'rt') as index_file:
        while pkg.parse_from_file(index_file):
            if pkg.source == srcpkg:
                pkg_list.append(pkg.copy())
...
def dpkg_fetch_unpack(srcpkg: str, builddir: str,
    ...
    files_sysdep = dict()
    for pkg in pkg_list:
        files = pkg.download_and_extract(base_repo_url, builddir)
