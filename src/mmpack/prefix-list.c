 ...
 *
 * Return: 0 in case of success, -1 otherwse with error state set accordingly.
 */
LOCAL_SYMBOL
int load_other_prefixes(struct strset* set, const char* ignore_prefix)
{
	...
	prefix_abspath = mmstr_abspath(ignore_prefix);
...
int update_prefix_list_with_prefix(const char* prefix)
{
	...
	list_load(&set);
	prefix_abspath = mmstr_abspath(prefix);
