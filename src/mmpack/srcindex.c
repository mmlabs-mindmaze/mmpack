...
mmstr* set_keyid(mmstr* keyid, const mmstr* srcname,
                 const struct sha_digest* srcsha)
{
	int len;
	...
	len = mmstrlen(keyid);
	sha_digest_to_hexstr(keyid + len, srcsha);
...
static
mmstr* create_keyid(const mmstr* srcname, const struct sha_digest* srcsha)
{
...
int srcpkg_setfield(struct srcpkg* pkg, struct strchunk line)
{
	...
	if (STR_EQUAL(key.buf, key.len, "size")) {
		...
		hexstr_to_sha_digest(&pkg->remote_res->sha256, val);
