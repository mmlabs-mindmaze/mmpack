...
void srcindex_add_pkg(struct srcindex* srcindex, struct srcpkg* pkg)
{
	...
	mmstr * name_id = NULL;
	mmstr * underscore = mmstr_malloc_from_cstr("_");

	name_id = mmstr_copy_realloc(name_id, pkg->name, mmstrlen(pkg->name));
	name_id = mmstrcat_realloc(name_id, underscore);
	name_id = mmstrcat_realloc(name_id, pkg->sha256);

	entry = indextable_lookup_create(&srcindex->pkgname_idx, name_id);
	*((struct srcpkg*) entry->value) = *pkg;

	mmstr_free(underscore);
	mmstr_free(name_id);
}
