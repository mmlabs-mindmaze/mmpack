...
/**
 * DOC: mounting prefix on Windows
 *
 ...
 * kernel looks up a path, it resolves the drive letter name by searching the
 * process, session and global device map (in this order). The first symbolic
 * link found with the searched name is used to resolve the drive part. Hence
 * if the process device map has the M: symbolic link, this will be used before
 * any session or global configuration. The process device map is setup using
 * NtSetInformationProcess with the ProcessDeviceMap argument.
 *
 * References:
