...
/**
 * DOC: Mounting mmpack prefix on Windows
 *
 * On Windows platform, the prefix is "mounted" onto the mmpack drive prefix
 * (M:) by settings of the process device map and hooking the process creation
 * API to inject process devicemap code in the child process.
 *
 * A device map is simply a NT directory object enclosing a NT symbolic link
 * named after the drive letter and pointing the resolved NT path. When the
 * kernel looks up a path, it resolves the drive letter name by searching in
 * the process, session and global device map (in this order). The first
 * symbolic link found with the searched name is used to resolve the drive
 * part. Hence if the process device map has the M: symbolic link, this will be
 * used before any session or global configuration. The process device map is
 * configured using NtSetInformationProcess with the ProcessDeviceMap argument.
 *
 * However process device map is not inherited upon process creation. In order
 * to overcome this, the process creation APIs are hooked as soon as the
 * mount-mmpack-prefix.dll is loaded in a process. Whenever a child process is
 * created, the call to API is intercepted and the device map of the parent is
 * set up in the child process and the dll is injected in the child process.
 * Consequently, when a device map is initially configured using
 * mount-mmpack-prefix.dll, the device map becomes inherited in all children.
 *
 * References:
 * http://blog.airesoft.co.uk/2012/01/chroot-ing-in-windows-as-easy-as-a-b-c
 * https://googleprojectzero.blogspot.com/2015/10/windows-drivers-are-truely-tricky.html (see "The History of DosDevices" section)
 */
