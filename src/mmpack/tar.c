...
/**
 * tarstream_open() - open an archive stream
 * tar:         pointer to a tarstream object to initialize
 * filename:    path to archive to open
 *
...
/**
 * tarstream_close() - Close an archive stream
 * tar:         pointer to a tarstream object to close
 *
 * This close the internal archive stream and free up the associated resources.
 * No matter the result, the object pointed to by @tar will be deinitialized.
 *
 * return: 0 in case of success, -1 otherwise.
 */
LOCAL_SYMBOL
...
/**
 * tarstream_read_next() - Points to the next entry of the archive stream
 * tar:         pointer to a tarstream object to advance
 *
 * return: 0 if the next entry has been loaded, READ_ARCHIVE_EOF if the end has
 * been reached. In case of failure, -1 is returned and the mmlib error is set.
 */
LOCAL_SYMBOL
...
/**
 * tarstream_extract_into_buffer() - extract current tar entry into a buffer
 * @tar:        tarstream being extracted
 * @buffer:     pointer to buffer receiving the data of the current entry
 *
 * Return: 0 in case of success, -1 otherwise
 */
LOCAL_SYMBOL
