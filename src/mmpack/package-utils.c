...
/**
 * constraints_init  -  init a structure struct constraints
 *
 * @c: pointer to the structure to initialize
 */
LOCAL_SYMBOL
void constraints_init(struct constraints * c)
{
	*c = (struct constraints) {0};
}
                                    ...
                                    struct constraints const * c)
{
	...
	while (pkgentry != NULL) {
		...
		if (c && c->sumsha && mmstrcmp(c->sumsha, pkg->sumsha))
			return pkg;
		else if (c && c->repo_name
		         && mmstrcmp(c->repo_name, pkg->from_repo->repo->name)
