...
enum constraints_type get_constraints_type(const char * key)
{
	...
	printf("In pkg_name=key:value, key not known\n");
	return UNKNOWN;
}
                    ...
                    const char * key_value)
{
	...
	colon = strchr_or_end(key_value, ':');
	if (*colon++ != ':') {
		cons->version = mmstrcpy_cstr_realloc(cons->version, key_value);
		return 0;
	}

	if ((type = get_constraints_type(key_value)) == UNKNOWN)
		return -1;


	switch (type) {
	case HASH:
		cons->sumsha = mmstrcpy_cstr_realloc(cons->sumsha, colon);
		break;
	case REPO:
		cons->repo = repolist_lookup(&ctx->settings.repo_list, colon);
		if (!cons->repo) {
		...
		break;
	case PKG_VERSION:
		cons->version = mmstrcpy_cstr_realloc(cons->version, colon);
		break;
	default:
		printf("In pgk_name=key:value, key not known\n");
		return -1;
	}
