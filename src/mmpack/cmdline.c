 ...
 * case no option is possible). "key:" can be either equal to "hash:" or to
 * "repo:". In case where "key:" is empty, then value corresponds to the version
 * of the package.
 *
                 ...
                 struct pkg_parser * pp)
{
	...
	if (*equal++ == '=') {
		...
		constraints_set(ctx, &pp->cons, type, colon);
	}
