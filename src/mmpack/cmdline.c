...
struct cmdline_constraints * parse_cmdline(const char* pkg_req)
{
	...
	struct cmdline_constraints * r = malloc(sizeof(struct cmdline_constraints));
	...
	if (first_sep == NULL && second_sep == NULL)
		...
		if (second_sep == NULL)
			...
			if (!strncmp(second_sep - strlen("sumsha:"), "sumsha:", 
			             strlen("sumsha:")))
				r->pkg_sumsha = mmstr_malloc_from_cstr(second_sep + 1);
			else if (!strncmp (second_sep - strlen("fromrepo:"),
			         "fromrepo:", strlen("fromrepo:")))
				r->repo_name = mmstr_malloc_from_cstr(second_sep + 1);	
			else {
