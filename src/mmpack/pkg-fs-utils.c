 ...
 * one of the file does not match its expected hash or an error occurred.
 */
static
...
int fschange_pkg_unpack(struct fschange* fsc, const mmstr* datapath)
{
	...
	if (fsc->curr_action->flags & ACTFL_FROM_PREFIX) {
		...
		path = sha256sums_path(NULL, fsc->curr_action->pkg);
		strlist_add(&fsc->inst_files, path);

		path = mmstr_asprintf(path, "%s/0", datapath);
		read_sumsha_filelist(path, &fsc->inst_files);
...
mmstr* imported_data_unpack_dir(mmstr* dir, const struct binpkg* pkg)
{
	...
	len = strlen(UNPACK_CACHEDIR_RELPATH) + 1 + SHA_HEXLEN;
...
static
int import_pkgs_from_prefix(struct mmpack_ctx* ctx,
                            ...
                            enum import_method method)
{
	...
	rv = hashset_init_from_file(&hset, path);
	mmstr_free(path);
	if (rv)
		return -1;
	...
	for (i = 0; i < act_stk->index; i++) {
		...
		if (!hashset_contains(&hset, &pkg->sumsha)
		    || import_binpkg_from_prefix(pkg, prefix, method,
		                                 act->pathname)) {
			// Add to the count of package to be installed but
			// could not be fetched from other prefix so far.
			num++;
			continue;
		}
		act->flags |= ACTFL_FROM_PREFIX;
...
static
void import_pkgs_from_other_prefixes(struct mmpack_ctx* ctx,
                                     struct action_stack* act_stk)
{
	...
	for (; prefix; prefix = strset_iter_next(&iter)) {
		if (!import_pkgs_from_prefix(ctx, act_stk, prefix, USE_HARDLINK))
			goto exit;
	}
 ...
 *
 * This downloads the mpk files needed for install or upgrade unless they can
 * be found and get successfully imported from an other known prefixes that are
 * the target packages installed.
 *
...
int fetch_pkgs(struct mmpack_ctx* ctx, struct action_stack* act_stk)
{
	...
	import_pkgs_from_other_prefixes(ctx, act_stk);

	// download mpk file for packages that could be be imported
	for (i = 0; i < act_stk->index; i++) {
		act = &act_stk->actions[i];
		pkg = act->pkg;
		if (act->action == REMOVE_PKG
