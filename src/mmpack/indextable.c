 ...
 * allocated and made available for biggest chain to accommodate an extra
 ...
 * possibility.
 *
 * Return: the created entry (fields are not initialized yet) in case of
...
int indextable_alloc(struct indextable* table)
{
	...
	table->buckets = xx_mm_aligned_alloc(64,
	                                     nmax*sizeof(*(table->buckets)));
	...
	// Initialize the list of available bucket node
                    ...
                    const struct indextable* restrict src)
{
	...
	*table = *src;
	...
	table->buckets = xx_mm_aligned_alloc(64, size);
