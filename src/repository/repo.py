...
def _init_logger(log_file: str) -> logging.Logger:
    ...
    Args:
        log_filename: name of the file where the log are stored.
    """
    ...
    log_file_basename = os.path.basename(log_file).split('_')[0]

    logger = logging.getLogger(log_file_basename + '-mmpack-repo')
...
class Repo:
    ...
    def __init__(self, sas: str, repo: str, architecture: str):
        ...
        self.working_dir = os.path.abspath(os.path.join(abs_path,
                                                        RELPATH_WORKING_DIR))
        open(os.path.join(self.repo_dir, LOG_FILE), 'w+')
