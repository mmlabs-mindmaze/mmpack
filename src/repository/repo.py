...
class _BinPkg:
    ...
    def write_keyvals(self, stream: TextIO):
        ...
        for key in ('version', 'source', 'srcsha256', 'ghost',
                    'sumsha256sums', 'file', 'size', 'sha256'):
...
class Repo:
    ...
    def _write_keyvals_binindex(self):
        binfile = os.path.join(self.working_dir, RELPATH_BINARY_INDEX_GZ)

        with gzip.open(binfile, 'wt', newline='\n') as stream:
