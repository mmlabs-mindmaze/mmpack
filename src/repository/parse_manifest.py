...
def _add_keys_in_dict(data: dict, binary_ind: str, source_ind: str,
    ...
    #update source-index
    #data_src_ind[data['name']] = {'filename': data['source']['file'],
    ...
    for arch in data['binpkgs']:
        for key in data['binpkgs'][arch]:
            ...
            with tarfile.open(arch_name, 'r:*') as arch:
                buf = yaml.safe_load(arch.extractfile('./MMPACK/info').read())
    ...
    with open(source_ind, 'w+') as source_file:
        yaml.dump(data_src_ind, source_file)
    with open(binary_ind, 'w+') as binary_file:
        yaml.dump(data_bin_ind, binary_file)
